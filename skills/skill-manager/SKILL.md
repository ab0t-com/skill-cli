...
```
skill                          status (default — like `git status`)
skill setup                    first-run: install all + shim + PATH + doctor
skill list                     same as status                              (alias: ls)
skill install [name|--all|--tag T|--category C|--profile P]   symlink SELECTED skills (stop brute-forcing all)
skill profile <list|show|add|rm>   manage named skill-sets (curated selections = groups for agents)   (agent: list/show --json)
...
skill rename <dir> <new>       rename a skill dir + update its symlink atomically
skill open <name>              open SKILL.md in $EDITOR
skill new <name>               scaffold a new skill
skill audit [name|--all]       LLM-backed quality audit (uses prompts/skill-audit-rubric.md)
skill secrets [name|--all]     secret-leak scan with gitleaks (--git for full history)   (alias: gitleaks, leaks)
skill snapshot [name|--all]    re-capture references/openapi.json from each skill's source_url  (alias: snap)
skill drift [name|--all]       check if bundled openapi.json drifted from live (read-only)      (alias: check)
skill prompt <action>          manage the prompt library (list|show|body|edit|history|new|render)
skill manifest                 JSON catalog of installed skills (name + description)
skill discover "<prompt>"      LLM router: pick skill(s) for a user prompt; auto-files missing-skill tickets   (alias: route)
skill tag <name> [+t -t | --set t...]     manage a skill's tags as a SET (add / remove / replace)
skill categorize <name> <cat> [tags...]   set category (+ ADD tags) in frontmatter (deterministic)
skill classify <name> [--apply]           LLM-suggest a category + tags (prefers taxonomy.yml); --apply writes it
skill tags [tag]                           tag inventory, or — with a tag — the skills carrying it   (agent: --json)
skill categories [cat]                     category inventory, or — with a category — the skills in it
...
Organizing skills:
- **Tags are a set of strings.** `skill tag <name> money api` ADDS (union, deduped); `-api` removes;
...
- **Browse/filter**: `skill tags <tag>` and `skill categories <cat>` list the matching skills;
  `skill list --tag T --category C` filters the status list (both support `--json` for agents).
...
every turn): `skill install --tag/--category/--profile` installs only a subset; **profiles** are
...
- `-h` / `--help` — works at top level (`skill --help`) AND per-command (`skill audit --help`, `skill scan --help`, etc.). Per-command help shows usage, behavior, env vars, and examples specific to that command.

Configuration (layered — nothing is hardcoded in command code):

    built-in defaults  ←  config.json (the BASE)  ←  env vars (override)

The BASE settings live in `<canonical-dir>/config.json` (run `skill config init`, or `skill setup`
writes it). View the resolved values + their source with `skill config`. The **canonical dir**
...
- All clean → no hint
- Nothing installed → `first time? run: skill setup`
- Some missing or conflicting → `to install/repair: skill install --all`
...
```bash
skill new my-new-skill        # scaffolds dir + SKILL.md template
skill open my-new-skill       # edit in $EDITOR
skill install my-new-skill    # symlink it into the target
```
...
```bash
skill audit                       # all skills with default model (Haiku)
skill audit billing               # one skill
...
```bash
skill secrets                            # all skills, current files, skip .snapshots/
skill secrets billing                    # one skill
skill secrets --git                      # also scan git history (slower, deeper)
...
```bash
skill snapshot                        # refresh all snapshots (idempotent)
skill snapshot billing                # one skill
skill snapshot --dry-run              # preview without writing
skill drift                           # read-only: compare snapshots vs live
skill drift billing                   # one skill
```
...
```bash
skill prompt list                              # all prompts (filters .example.md)        (alias: ls)
skill prompt show <name>                       # full file (frontmatter + body)           (alias: cat)
skill prompt body <name>                       # body pre-render (Jinja2 syntax visible)
skill prompt render <name>                     # body post-Jinja2 with empty vars
skill prompt render <name> --vars '<json>'     # render with inline vars
skill prompt render <name> --vars-file <path>  # vars from a file (use - for stdin)
skill prompt edit <name>                       # open in $EDITOR                          (alias: open)
skill prompt history <name>                    # git log for that prompt's file           (alias: log)
skill prompt new <name>                        # scaffold v2 frontmatter + body template  (alias: create)
```
...
When the catalog grows past ~50 skills, naive description-matching at the harness level breaks down. `skill discover` adds an LLM routing layer:

```bash
skill discover "I need to set up DataDog APM for the billing service"
```

The CLI builds a JSON manifest of installed skills (`skill manifest`), feeds it to the `skill-discovery` prompt with the user prompt, and returns one of three decision branches:
...
**The `missing-skill` workflow accumulates a backlog of skills worth writing.** Each ticket has the full context needed to dispatch `create-service-api-reference-skill` (or a similar scaffolder) when the proposal is ready to materialize. Re-running `skill discover` on the same prompt is deduplicated by `proposed_name` — no spam tickets.
...
`skill doctor` checks four things:

| Category | Auto-fixable? | Fix |
|---|---|---|
| **Broken symlink** in target (points to a missing source) | yes (`--fix` removes it) | `skill doctor --fix` |
| **Dirname / frontmatter mismatch** (dir is `foo_bar`, frontmatter says `foo-bar`) | no — too destructive to auto-rename | `skill rename <dir> <newname>` (printed as a hint) |
| **Missing `name:` in frontmatter** | no — needs human input | edit the SKILL.md and add the field |
| **Missing `description:` in frontmatter** | no — needs human-written description | `skill open <name>` and write one |
